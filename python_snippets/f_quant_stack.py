...
class TestBoundedStack:
   ...
   def test_push_when_full(self) -> None:
       stack = BoundedStack(_max_size=1)
       stack = stack.push(1)
       stack = stack.push(2)
   ...
   def test_immutability(self, stack_with_items: BoundedStack[int]) -> None:
       original_size = stack_with_items.size()
       stack_with_items.push(3)
       assert stack_with_items.size() == original_size

       another_push = stack_with_items.push(4)
       assert stack_with_items is not another_push

   def test_type_safety(self) -> None:
       stack: BoundedStack[str] = BoundedStack()
